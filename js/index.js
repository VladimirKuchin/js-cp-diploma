document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < navDay.length; i++) {
    let todayDate = new Date(today + 24 * 60 * 60 * 1000 * i);
    dayWeek[i].textContent = getDay(todayDate); 
    dayNumber[i].textContent = todayDate.getDate(); 

    if(getDay(todayDate) === "Вс" || getDay(todayDate) === "Сб"){
